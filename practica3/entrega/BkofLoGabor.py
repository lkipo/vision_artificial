...
class BkofLoGabor:
    ...
    def roseta(self):
        ...
        for filter in self.__total_filters:
            ...
            total = cv2.add(total, ifftshift(filter)) # sumamos cada filtro á imaxe total
    ...
    def roseta_corte(self, margin=0.02):
        ...
        for filter in self.__total_filters:
            range = cv2.inRange(filter, 0.67-margin, 0.67+margin).astype(np.float64) # miramos onde corta a roseta co 0.67
            range = range

            total = cv2.add(total, ifftshift(range)) # sumamola ao total
        plt.imshow(total)
        plt.show()
        
    def spacial_filters(self):
        for filter in self.__total_filters:
            plt.imshow(ifftshift(np.real(ifft2(filter)))) # mostramos a transformada inversa shifteada do filtro no dominio frecuencial
            plt.show()
    ...
    def radial_symmetry(self):
        ...
        for i in range(self.nscale):
            ...
            scale = self.__bank[:, i]
            mult_orientation = np.ones_like(self.__bank[0][0]) # aqui gardaranse o resultado das multiplicacións para cada orientación
            
            for orient in scale:
                mult_orientation = mult_orientation * np.abs(orient)
            
            mult_scale = np.maximum(mult_scale, mult_orientation) # sumamos todas as escalas
        
        plt.imshow(mult_scale)
        plt.show()
        
        # DESCOMENTAR A SEGUINTE LIÑA PARA GARDAR A IMAXE
    ...
    def localeg_orient(self):
        ...
        for orient in self.__bank:
            ...
            for filter in orient:
                total  = np.maximum(np.sqrt((filter.real**2)+(filter.imag**2)), total) # calculamos a enerxía local de cada escala e sumamola ao total
    ...
    def localeg_scale(self):
        for i in range(len(self.__bank[0])):
            scale = self.__bank[:,i,:,:] # obtemos todas as respostas dunha mesma escala
            total = np.zeros_like(self.__bank[0][0])
            # para cada escala...
            for filter in scale:
                total  = np.maximum(np.sqrt((filter.real**2)+(filter.imag**2)), total) # calculamos a enerxía local de cada orientación e sumamola ao total
            
            plt.imshow(total)
            plt.show()
            
    def localeg_total(self):
        # para cada orientación...
        total = np.zeros_like(self.__bank[0][0])
        for orient in self.__bank:
            totalor = np.zeros_like(self.__bank[0][0])
            for filter in orient:
                totalor  = np.maximum(np.sqrt((filter.real**2)+(filter.imag**2)), total) # calculamos a enerxía local de cada escala e sumamola ao total
            total = np.maximum(totalor, total)
        plt.imshow(total)
        plt.show()
        
    def localphase_orient(self):
        # para cada orientación...
        for orient in self.__bank:
            total = np.zeros_like(self.__bank[0][0])
            for filter in orient:
                total  = np.maximum(np.arctan2(filter.imag, filter.real), total) # calculamos a fase local de cada escala e sumamola ao total
                            
            plt.imshow(total)
            plt.show()

    def localphase_scale(self):
        for i in range(len(self.__bank[0])):
            scale = self.__bank[:,i,:,:] # obtemos todas as respostas dunha mesma escala
            total = np.zeros_like(self.__bank[0][0])
            # para cada escala...
            for filter in scale:
                total = np.maximum(np.arctan2(filter.imag, filter.real), total) # calculamos a fase local de cada orientación e sumamolo ao total
                            
            plt.imshow(total)
            plt.show()
        
    def localphase_total(self):
        total = np.zeros_like(self.__bank[0][0])
        # para cada orientación...
        for orient in self.__bank:
            totalor = np.zeros_like(self.__bank[0][0])
            for filter in orient:
                totalor  = np.maximum(np.arctan2(filter.imag, filter.real), total) # calculamos a fase local de cada escala e sumamola ao total
            total = np.maximum(total, totalor)
